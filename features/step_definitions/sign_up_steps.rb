Given(/^a new user "(.*?)"$/) do |name|
  @user = User.new(name: name, password: "secret123", email: "1@mail.me")
end
...
When(/^he signs up with a username that already exists$/) do
  User.create!(name: @user.name, password: "secret123", email: "2@mail.me")
  sign_up(@user.name, @user.password, @user.email)
end

When(/^he signs up with an email that is already in use$/) do
  User.create!(name: "elad", password: "secret123", email: "1@mail.me")
  sign_up(@user.name, @user.password, @user.email)
end